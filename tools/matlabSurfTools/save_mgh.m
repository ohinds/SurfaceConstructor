function r = save_mgh(vol, fname, M, mr_parms, format);
%
...
%
% $Id: save_mgh.m,v 1.2 2005/10/06 22:48:18 oph Exp $
%
...
if(ndims(vol) == 5)
  ...
  is_tensor = 0 ;
  fwrite(fid, format, 'int') ;  % type = MRI_FLOAT
end
...
if(format == MRI_UCHAR) 
  fwrite(fid,vol,'uchar');
elseif(format == MRI_SHORT)
  fwrite(fid,vol,'short');
elseif(format == MRI_INT | format == MRI_LONG) 
  fwrite(fid,vol,'int');
else
  mx = [];
